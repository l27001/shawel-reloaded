...
def send_to_users(prefix, dir, txt):
	...
	file_ids = []
	for n in bot.send_media_group(-1001716874276, to_send):
	...
	for user in users:
		...
		try:
			...
			bot.send_message(user['tgid'], txt, "HTML", None, True, False)
	...
	for chat in chats:
		...
		try:
			...
			bot.send_message(chat['id'], txt, "HTML", None, True, False)
		except Exception as e:
			log("TG_ERR", str(e))
